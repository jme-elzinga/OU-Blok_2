...
public class Voorstelling {
   ...
   */
  protected void wijzigStatusPlaats(int rij, int stoel, Status status) {
    if(rij>0 && rij<=Theater.AANTALRIJEN && stoel>0 && stoel<=Theater.AANTALPERRIJ && !plaatsen[rij][stoel].getStatus().equals(Status.BEZET) && !status.equals(Status.BEZET)) {
   ...
   */
  protected void resetReserveringen() {
    for(int i=1;i<=Theater.AANTALRIJEN; i++) {
      for (int j=1; j<=Theater.AANTALPERRIJ; j++) {
        if(plaatsen[i][j].getStatus().equals(Status.GERESERVEERD)) {
   ...
   */
  protected int getAantalPlaatsen(Status status) {
    int teller=0;
    for(int i=1;i<=Theater.AANTALRIJEN; i++) {
      for (int j=1; j<=Theater.AANTALPERRIJ; j++) {
